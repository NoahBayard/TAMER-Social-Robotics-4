...
def main(saved_models_dir, render=True, seed_list=None):
    """
    Load all trained TAMER models from the saved_models directory and play one episode for each model using given seeds.
    ...
    for model_file, seed in zip(model_files, seed_list):
        ...
        tamer_agent = Tamer(environment, num_episodes=1, seed=seed)  # Pass the seed to the agent
...
if __name__ == "__main__":
    ...
    saved_models_dir = '/Users/noahbayard/Downloads/TAMER-main/saved_taxi_models'

    # Define the seed list that matches the training seeds
    seed_list = [1, 1, 1]  # Replace with the actual seed values used during training

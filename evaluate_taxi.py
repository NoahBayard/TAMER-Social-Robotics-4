...
from tamer.player_taxi import Tamer
import gymnasium as gym
import pickle

MODELS_DIR = '/Users/noahbayard/Downloads/TAMER-main/saved_taxi_models'
...
def evaluate_models(env, models_dir, num_episodes, seed_list):
    ...
    for model_file, seed in zip(model_files, seed_list):
        ...
        agent = Tamer(env, num_episodes, tame=True, seed=seed)  # Set the seed for each agent
        agent.H = model

        # Evaluate the agent and calculate the average reward for the model
        episode_times, avg_reward, avg_duration, avg_timesteps = agent.evaluate(n_episodes=num_episodes)
...
def plot_average_rewards(avg_rewards):
    ...
    plt.savefig('average_rewards_plot_taxi_tamer.png')
    print("Plot saved as 'average_rewards_plot_taxi_tamer.png'")
